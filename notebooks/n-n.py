...
def main(args):
    ...
    triple = open(os.path.join(input_dir, 'train2id.txt'), 'r')
    valid = open(os.path.join(input_dir, 'valid2id.txt'), 'r')
    test = open(os.path.join(input_dir, 'test2id.txt'), 'r')
    ...
    sn1 = 0
    snn = 0
    f = open(os.path.join(input_dir, 'test2id.txt'), 'r')
    tot = (int)(f.readline())
    for i in range(tot):
        content = f.readline()
        h, t, r = content.strip().split()
    ...
    fall = open(os.path.join(output_dir, 'test2id_all.txt'), 'w')
    tot = (int)(f.readline())
    fall.write('%d\n' % (tot))
    f11.write('%d\n' % (s11))
    f1n.write('%d\n' % (s1n))
